...
| `name` | `name` | |
| `ownerId` | `acctmgr` | **ADDED 2026-08-02; push ADDED 2026-08-05.** Company owner is a free-text account-manager NAME (not a user id). Resolved to a local user by unique last name + first initial (`matchSageUserByName`). Unmatched names (former reps, blanks, junk) leave the company owner-less. Only 3 current reps appear as `acctmgr`. Local Owner edits push back to Sage `acctmgr` as `"FirstName LastName"` via `acctMgrNameForEmail` when the owner is a mapped rep; unmapped/null owners omit the field (do not wipe Sage). |
...
| `DealQuote.quoteNumber` | `note` / quoting-tool stamp, else `description` | **ADDED 2026-09-11** — child rows, not a scalar. Canonical `QYYMMDD-###` only. Expand Sage shorthand (`#Q260622-003,4,5,6,7` → five ids) before insert. Never store quote-tool `SavedQuote.id`. Pull is additive (does not delete human / `PO_TOOL` rows). `isPrimary` is a cross-deal disambiguator (one true row per colliding `quoteNumber`), not “first quote on this deal”. Analytics extract is one row per (`dealId`, `quoteNumber`); join `upper(trim(quoteNumber))` = `fct_quotes.quote_number`. Unique numbers stay false (`deal_quote_exact`). One quote on many deals stays `ambiguous` unless exactly one `isPrimary` (`deal_quote_primary`). Do not parse names in the warehouse. |
