...
| `DealQuote.quoteNumber` | `note` / quoting-tool stamp, else `description` | **ADDED 2026-09-11** — child rows, not a scalar. Canonical `QYYMMDD-###` only. Expand Sage shorthand (`#Q260622-003,4,5,6,7` → five ids) before insert. Never store quote-tool `SavedQuote.id`. Pull is additive (does not delete human / `PO_TOOL` rows). Optional `isPrimary` for the deal card. Analytics extract is one row per (`dealId`, `quoteNumber`); join `upper(trim(quoteNumber))` = `fct_quotes.quote_number`. One quote on many deals stays `ambiguous` unless exactly one `isPrimary`. Do not parse names in the warehouse. |
