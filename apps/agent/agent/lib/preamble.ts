...
export async function tripPreamble(
	...
	const markdown = [
		...
		`- **Rank nearby companies** → \`search_trip_candidates\` with tripPlanId \`${plan.id}\`. Returns a capped ranked list (must-visits first, then **open-deal accounts by deal size**, then other activity, then distance). Never invent companies.`,
		...
		"**Fill the trip with open deals:** after must-visits, schedule other nearby accounts that have open pipeline — largest `openPipelineAmount` first. Do not skip a large open deal in the radius to visit a smaller or inactive account.",
