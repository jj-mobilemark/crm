...
export async function tripPreamble(
	...
	const markdown = [
		...
		`- **Rank nearby companies** → \`search_trip_candidates\` with tripPlanId \`${plan.id}\`. Returns a capped ranked list (must-visits first, then **ownership** mine → unassigned → other, then **open-deal accounts by deal size**, then other activity, then distance). Never invent companies.`,
		...
		"**Fill the trip with the rep's own open deals:** after must-visits, schedule nearby `ownership=mine` accounts that have open pipeline — largest `openPipelineAmount` first. Unassigned accounts may fill leftover slots without asking.",
		"**Other owners:** when `ownership=other` accounts look like strong visit candidates (high open pipeline or strong salvage signal), name the company and `ownerName` and ask the rep with `ask_question` whether they want to visit that customer even though another owner is on it. Do **not** add other-owned stops to the itinerary until the rep agrees.",
		"",
		capabilitiesMarkdown(),
	]
		.filter(Boolean)
