 ...
 *
 * The twin of Google's `ConversationService`. It is provider-agnostic on
 * purpose: a thread may hold Gmail-stored messages, Outlook-stored ones, or a
 * mix, so it selects both id fields and returns both deep-link forms. One
 * service can serve `google.thread` and `microsoft.thread` alike.
 *
 * Separate from the sync services because this is the only path that returns
 * message bodies, so "where can an email body reach the browser" has one
 * answer. The timeline never calls it — it carries a snippet, and the accordion
 * fetches this on expand.
 */
@Injectable()
